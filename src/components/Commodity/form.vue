    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline" label-width="90px">
      ...
      <el-form-item label="品牌:">
        <el-select v-model="formInline.brand_id" placeholder="请选择">
          <el-option v-for="item in formSelect.brand" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="商品来源:">
        <el-select v-model="formInline.prod_src" placeholder="请选择">
          <el-option v-for="item in formSelect.source" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="商品类型:">
        <el-select v-model="formInline.prod_type" placeholder="请选择">
          <el-option v-for="item in formSelect.product_type" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="配送方式:">
        <el-select v-model="formInline.type" placeholder="请选择">
          <el-option v-for="item in formSelect.delivery" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item v-show="activeName==3||activeName==1||activeName==4" :label="activeName==4?'上架时间:':(activeName==1?'提交时间:':(activeName==3?'审核时间:':''))">
        <el-date-picker
      ...
      <el-form-item v-show="activeName==5" label="下架原因:">
        <el-select v-model="formInline.down_reason" placeholder="请选择">
          <el-option v-for="item in formSelect.down_reason" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item v-show="activeName==5" label="供货商状态:">
        <el-select v-model="formInline.retailer_status" placeholder="请选择">
          <el-option v-for="item in formSelect.retailer_status" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
